...
func TestLeafHandler_FailurePaths(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			respBytes, appErr := h.AppRequest(t.Context(), ids.GenerateTestNodeID(), time.Time{}, synctest.MustMarshal(t, &syncpb.GetLeafsRequest{}))
			if tt.wantAppErr {
	...
	tests := []struct {
	...
	}{
		{
			...
			req: &syncpb.GetLeafsRequest{
				RootHash: root.Bytes(),
				KeyLimit: 10,
				NodeType: syncpb.NodeType_NODE_TYPE_UNSPECIFIED,
			},
		},
		{
			...
			req: &syncpb.GetLeafsRequest{
				RootHash: root.Bytes(),
				KeyLimit: 10,
				NodeType: syncpb.NodeType_NODE_TYPE_ATOMIC_TRIE,
			},
		},
		{
			name: "zero KeyLimit",
...
func TestLeafResponder_MissingRootDrops(t *testing.T) {
	...
	resp, err := r.Respond(t.Context(), ids.GenerateTestNodeID(), &syncpb.GetLeafsRequest{
...
func TestLeafResponder_CtxCancelledDrops(t *testing.T) {
	...
	r := handlers.NewLeafResponder(trieDB, common.HashLength, nil, &synctest.LeafRecorder{})
	resp, err := r.Respond(ctx, ids.GenerateTestNodeID(), &syncpb.GetLeafsRequest{
		RootHash: root.Bytes(),
		KeyLimit: 10,
		NodeType: syncpb.NodeType_NODE_TYPE_STATE_TRIE,
	})
