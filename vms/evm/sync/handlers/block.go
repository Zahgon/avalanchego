...
type BlockHandler = Handler[*syncpb.GetBlockRequest, *syncpb.BlockResponse]

// BlockResponder is the inner contract for block-batch requests.
type BlockResponder = Responder[*syncpb.GetBlockRequest, *syncpb.BlockResponse]
...
// BlockProvider returns blocks by hash and height. nil stops the
// parent walk.
...
// blockResponder walks the parent chain from the requested hash via a
// [BlockProvider].
...
func (r *blockResponder) Respond(ctx context.Context, nodeID ids.NodeID, req *syncpb.GetBlockRequest) (*syncpb.BlockResponse, error) {
	startTime := time.Now()
	...
	hash := common.BytesToHash(req.GetHash())
	...
	for range parents {
		...
		block := r.blocks.GetBlock(hash, height)
		if block == nil {
	...
	if len(encoded) == 0 {
		log.Debug("no requested blocks found, dropping request", "nodeID", nodeID, "hash", req.GetHash(), "parents", req.GetParents())
		return nil, nil
	}
	return &syncpb.BlockResponse{Blocks: encoded}, nil
}
