...
// LeafHandler serves [syncpb.GetLeafsRequest] over [p2p.EVMLeafsRequestHandlerID].
type LeafHandler = Handler[*syncpb.GetLeafsRequest, *syncpb.LeafsResponse]

// LeafResponder is the inner contract for leaf-range requests.
type LeafResponder = Responder[*syncpb.GetLeafsRequest, *syncpb.LeafsResponse]

// NewLeafHandler wires resp into a [LeafHandler].
func NewLeafHandler(resp LeafResponder) *LeafHandler {
	return NewHandler(func() *syncpb.GetLeafsRequest { return &syncpb.GetLeafsRequest{} }, resp)
}
...
func (r *leafResponder) Respond(ctx context.Context, nodeID ids.NodeID, req *syncpb.GetLeafsRequest) (*syncpb.LeafsResponse, error) {
...
func validateLeafRequest(req *syncpb.GetLeafsRequest, trieKeyLength int) bool {
	if req.GetNodeType() != syncpb.NodeType_NODE_TYPE_STATE_TRIE {
		return false
	}
...
func newLeafBuild(r *leafResponder, nodeID ids.NodeID, req *syncpb.GetLeafsRequest) *leafBuild {
	startTime := time.Now()
...
func (b *leafBuild) run(ctx context.Context, nodeID ids.NodeID) (*syncpb.LeafsResponse, error) {
	defer func() {
