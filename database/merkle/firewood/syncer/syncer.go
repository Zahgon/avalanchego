...
func New(config Config, db *ffi.Database, targetRoot ids.ID, rangeProofClient *p2p.Client, changeProofClient *p2p.Client) (*sync.Syncer[*RangeProof, *ChangeProof], error) {
	return newWithDB(
		config,
		&database{db: db},
		targetRoot,
...
func newWithDB(config Config, db sync.DB[*RangeProof, *ChangeProof], targetRoot ids.ID, rangeProofClient *p2p.Client, changeProofClient *p2p.Client) (*sync.Syncer[*RangeProof, *ChangeProof], error) {
